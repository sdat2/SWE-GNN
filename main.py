...
def main(config):
    ...
    node_features, edge_features = temporal_train_dataset[0].x.size(
        -1
    ...
    optimizer = optim.Adam(
        model.parameters(),
        lr=lr_info["learning_rate"],
        weight_decay=lr_info["weight_decay"],
    )
    lr_scheduler = optim.lr_scheduler.StepLR(
        optimizer=optimizer, step_size=lr_info["step_size"], gamma=lr_info["gamma"]
    )

    # Batch creation
    train_loader = DataLoader(
        temporal_train_dataset, batch_size=batch_size, shuffle=True
    )
    ...
    spatial_analyser = SpatialAnalysis(
        model, test_dataset, **temporal_test_dataset_parameters
    )
    ...
    for id_dataset, name in zip([best_id, worst_id], ["best", "worst"]):
        rollout_plotter = PlotRollout(
            model,
            test_dataset[id_dataset],
            scalers=scalers,
            type_loss=trainer.type_loss,
            **temporal_test_dataset_parameters,
        )
