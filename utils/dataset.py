...
def get_node_features(
    ...
    if scalers is None:
        scalers = {"DEM_scaler": None, "slope_scaler": None}
    ...
    if slope_x or slope_y:
        ...
        if slope_x:
            node_features["slope_x"] = process_attr(
                slopex, scaler=scalers["slope_scaler"], device=device
            )

        if slope_y:
            node_features["slope_y"] = process_attr(
                slopey, scaler=scalers["slope_scaler"], device=device
            )
...
def get_edge_features(
    ...
    selected_edges = [
        edge_features[key].reshape(-1, 1).to(device)
...
def create_data_attr(
    ...
    selected_node_features = {
        key: selected_features[key] for key in ["slope_x", "slope_y", "area", "DEM"]
    }

    selected_edge_features = {
        key: selected_features[key] for key in ["cell_length", "normal_x", "normal_y"]
    }
...
def create_model_dataset(
    ...
    **dataset_parameters,
    ...
    train_dataset = load_dataset(
        dataset_name, train_size, seed, dataset_folder + "/train"
    )
    ...
    if test_size == "big" or test_size == "big_random_breach":
        test_dataset = load_dataset(
            "big_random_breach_grid",
            10,
            seed=0,
            dataset_folder=dataset_folder + "/test",
        )
    elif test_size == "random_breach":
        test_dataset = load_dataset(
            "random_breach_grid", 20, seed=0, dataset_folder=dataset_folder + "/test"
        )
    else:
        test_dataset = load_dataset(
            dataset_name, test_size, seed=0, dataset_folder=dataset_folder + "/test"
        )
...
def get_next_steps(aggregate_function, init_time, rollout_steps, *water_variables_args):
    ...
    next_steps = torch.stack(
        [
            aggregate_function(*water_variables_args, step)
            for step in range(init_time, init_time + rollout_steps)
        ],
        -1,
    )
    assert (
...
def to_temporal(
    data, previous_t=2, time_start=0, time_stop=-1, rollout_steps=1, with_x=True
):
    ...
    for init_time in range(time_start, time_start + temporal_samples_size):
        ...
        output_dim = 2
        assert (
            prev_steps.shape[1] == output_dim * previous_t
        ), f"The output dimension is wrong: {prev_steps.shape}"
        assert (
            next_steps.shape[1] == output_dim
        ), f"The output dimension is wrong: {next_steps.shape}"
        if (
            prev_steps[:, -output_dim:] != 0
        ).all():  # Except when everything is zero, then no problem
            assert ~torch.isclose(
                prev_steps[:, -output_dim:], next_steps[:, :, 0]
...
def use_prediction(x, pred, previous_t):
    ...
    if static_vars != 0:  # equivalent to with_x == True
        ...
        if previous_t == 1:
            ...
            temp = torch.cat((x[:, out_dim:], pred), 1)
...
def get_temporal_test_dataset_parameters(config, temporal_dataset_parameters):
    try:
        ...
        temporal_test_dataset_parameters["with_x"] = temporal_dataset_parameters[
            "with_x"
        ]
    except:
