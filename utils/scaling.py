...
def stack_attrs(dataset, attr, to_min=False):
    ...
    """
    stacked_map = torch.stack(
        [data[attr] - data[attr].min() * to_min for data in dataset]
    )
...
def scaler(
    train_database, attr: str or list or tuple, type_scaler="minmax", to_min=False
):
    """
    ...
    if isinstance(attr, list):
        all_attrs = torch.cat(
            [stack_attrs(train_database, att, to_min=to_min) for att in attr]
        )
    elif isinstance(attr, tuple):
        all_attrs = torch.cat(
            [stack_attrs(train_database, att, to_min=to_min) ** 2 for att in attr], 1
        )
