...
def get_model(model_name):
    models_dict = {"GNN": GNN, "CNN": CNN, "MLP": MLP}
...
def get_numerical_times(
    ...
    **temporal_test_dataset_parameters,
    ...
    random_breach_test_id = numerical_simulation_overview["seed"].isin(
        np.arange(10001, 10021)
    )
    big_random_breach_test_id = numerical_simulation_overview["seed"].isin(
        np.arange(15001, 15011)
    )
...
def get_rollout_loss(
    ...
    if only_where_water:
        where_water = mask_on_water(
            predicted_rollout, real_rollout, water_axis=water_axis
        )
...
class SpatialAnalysis:
    def __init__(self, model, dataset, **temporal_test_dataset_parameters):
        ...
        self.predicted_rollout, self.real_rollout, self.prediction_times = (
            self._get_rollouts(model, dataset, **temporal_test_dataset_parameters)
        )
    ...
    def _get_rollouts(self, model, dataset, **temporal_test_dataset_parameters):
        if isinstance(dataset, list):
            all_rollouts = [
                get_rollouts(model, data, **temporal_test_dataset_parameters)
                for data in dataset
            ]
            ...
            predicted_rollout, real_rollout, prediction_times = get_rollouts(
    ...
    def _plot_metric_rollouts(
        ...
        for wt in water_thresholds:
            metric = (
                metric_function(
                    self.predicted_rollout, self.real_rollout, water_threshold=wt
                )
                .to("cpu")
                .numpy()
            )
    ...
    def _plot_rollouts(self, type_loss, ax=None):
        ...
        for var in range(self.diff_rollout.shape[2]):
            average_diff_t = (
                get_mean_error(self.diff_rollout, type_loss, nodes_dim=1)[:, var, :]
                .to("cpu")
                .numpy()
            )
    ...
    def plot_loss_per_simulation(
        ...
        axs[2].plot(
            CSIs[sorted_ids], "o--", label=[f"CSI_{str(wt)}" for wt in water_thresholds]
        )
        axs[2].set_ylim(0, 1)
