...
class BasePlotMap(object):
    ...
    def _check_map_type(self):
        if self.graph is None and self.mesh is None:
            raise AttributeError(
                "BasePlotMap must receive either a graph 'graph' or a triangular mesh 'mesh'"
            )
            ...
            if self.difference_plot:
                if self.kwargs["vmin"] >= 0:
                    ticks_interval = np.linspace(
                        0, self.kwargs["vmax"], 6, endpoint=True
                    )
                    plt.colorbar(
                        ...
                        * np.floor(np.abs(ticks_interval) * 10)
                        / 10,
                        fraction=0.05,
                        shrink=0.9,
                        ax=ax,
                    ...
                    ticks_interval = np.linspace(
                        self.kwargs["vmin"], 0, 6, endpoint=True
                    )
                    plt.colorbar(
                        plt.cm.ScalarMappable(
                    ...
                    ticks_interval = np.linspace(
                        self.kwargs["vmin"], self.kwargs["vmax"], 7, endpoint=True
                    )
                    plt.colorbar(
                        plt.cm.ScalarMappable(
                            norm=TwoSlopeNorm(
                ...
                ticks_interval = np.linspace(
                    self.kwargs["vmin"], self.kwargs["vmax"], 6, endpoint=True
                )
                plt.colorbar(
                    plt.cm.ScalarMappable(
                        norm=plt.Normalize(
                            vmin=self.kwargs["vmin"], vmax=self.kwargs["vmax"]
                        ),
                        cmap=self.kwargs["cmap"],
                    ),
                    ticks=np.sign(ticks_interval)
    ...
    def plot_map(self, ax=None, colorbar=True):
        ...
        if self.graph is not None:
            ...
            Y = coordinates[:, 1]
            ax.tricontourf(
                X, Y, self.mesh.simplices, self.map, levels=48, **self.kwargs
            )
...
class PlotRollout:
    ...
    def _get_maxs(self, real_rollout, predicted_rollout, diff_rollout):
        ...
        self.last_V_max = max(
            abs(predicted_rollout[:, 1:, -1]).max(), abs(real_rollout[:, 1:, -1]).max()
        )
    ...
    def _get_WDPlots(self, real_rollout, predicted_rollout, diff_rollout):
        ...
        difference_WD = TemporalPlotMap(
            diff_rollout[:, 0, :],
            **self.default_temporal_plot_kwargs,
            scaler=self.scalers["WD_scaler"],
            difference_plot=True,
            vmin=-self.max_diff_WD,
    ...
    def _get_VPlots(self, real_rollout, predicted_rollout, diff_rollout):
        ...
        difference_V = TemporalPlotMap(
            diff_rollout[:, 1, :],
            **self.default_temporal_plot_kwargs,
            scaler=self.scalers["V_scaler"],
            difference_plot=True,
            vmin=-self.max_diff_V,
    ...
    def compare_h_rollout(self, plot_times=[1, 6, 24, 40]):
        ...
        fig, axs = plt.subplots(
            3, n_plots, figsize=(n_plots * 4, 12), facecolor="white"
        )
        ...
        for axs_index, time_step in enumerate(self.plot_times, start=1):
            ...
            self.real_WD.plot_map(
                time_step=time_step, ax=axs[0, axs_index], colorbar=colorbar
            )
            self.predicted_WD.plot_map(
                time_step=time_step, ax=axs[1, axs_index], colorbar=colorbar
            )
            self.difference_WD.plot_map(
                time_step=time_step, ax=axs[2, axs_index], colorbar=colorbar
            )
...
class DoublePlotRollout(PlotRollout):
    ...
    def __init__(
        ...
        **temporal_test_dataset_parameters,
        ...
        self.predicted_rollout1, self.real_rollout, self.prediction_time1 = (
            get_rollouts(model1, dataset, **temporal_test_dataset_parameters)
        )
        ...
        self._get_maxs_12(
            self.real_rollout,
            self.predicted_rollout1,
            self.diff_rollout1,
            self.predicted_rollout2,
            self.diff_rollout2,
        )
    ...
    def _get_maxs_12(
        ...
        self.WD_max = max(
            real_rollout[:, 0, :].max(),
            predicted_rollout1[:, 0, :].max(),
            predicted_rollout2[:, 0, :].max(),
        )

        self.max_diff_WD = max(
            abs(diff_rollout1[:, 0, :]).max(), abs(diff_rollout2[:, 0, :]).max()
        )
        self.last_max_diff_WD = max(
            abs(diff_rollout1[:, 0, -1]).max(), abs(diff_rollout2[:, 0, -1]).max()
        )

        self.V_max = max(
            abs(predicted_rollout1[:, 1:, :]).max(),
            abs(predicted_rollout2[:, 1:, :]).max(),
            abs(real_rollout[:, 1:, :]).max(),
        )
        self.last_V_max = max(
            abs(predicted_rollout1[:, 1:, -1]).max(),
            abs(predicted_rollout2[:, 1:, -1]).max(),
            abs(real_rollout[:, 1:, -1]).max(),
        )

        self.max_diff_V = max(
            abs(diff_rollout1[:, 1:, :]).max(), abs(diff_rollout2[:, 1:, :]).max()
        )
        self.last_max_diff_V = max(
            abs(diff_rollout1[:, 1:, -1]).max(), abs(diff_rollout2[:, 1:, -1]).max()
        )
    ...
    def compare_h_rollout(self, plot_times=[1, 6, 24, 40]):
        ...
        n_plots = len(self.plot_times) + 1
        fig, axs = plt.subplots(
            5, n_plots, figsize=(n_plots * 4, 20), facecolor="white"
        )
        ...
        for axs_index, time_step in enumerate(self.plot_times, start=1):
            ...
            self.predicted_WD1.plot_map(
                time_step=time_step, ax=axs[1, axs_index], colorbar=colorbar
            )
            self.difference_WD1.plot_map(
                time_step=time_step, ax=axs[2, axs_index], colorbar=colorbar
            )
            self.predicted_WD2.plot_map(
                time_step=time_step, ax=axs[3, axs_index], colorbar=colorbar
            )
            self.difference_WD2.plot_map(
                time_step=time_step, ax=axs[4, axs_index], colorbar=colorbar
            )
            axs[0, axs_index].set_title(f"time: {self.real_WD.time_in_hours:.1f} h")
    ...
    def compare_v_rollout(self, plot_times=[1, 6, 24, 40]):
        ...
        for axs_index, time_step in enumerate(self.plot_times, start=1):
            ...
            self.predicted_V1.plot_map(
                time_step=time_step, ax=axs[1, axs_index], colorbar=colorbar
            )
            self.difference_V1.plot_map(
                time_step=time_step, ax=axs[2, axs_index], colorbar=colorbar
            )
            self.predicted_V2.plot_map(
                time_step=time_step, ax=axs[3, axs_index], colorbar=colorbar
            )
            self.difference_V2.plot_map(
                time_step=time_step, ax=axs[4, axs_index], colorbar=colorbar
            )
            axs[0, axs_index].set_title(f"time: {self.real_V.time_in_hours:.1f} h")
