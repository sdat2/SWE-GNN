...
def grid_DEM_generator(
    ...
    DEM = np.array(
        [
            [noise([i / xpix, j / ypix]) * DEM_multiplier for j in range(xpix)]
            for i in range(ypix)
        ]
    )
...
def save_DEM(DEM, grid_size, dst_file, pos=None):
    ...
    if pos is None:
        ...
        y_grid = np.array(
            [
                [(i + 0.5) * grid_size for j in range(number_grids)]
        ...
        xyz = np.array(
            [
                ...
                for (x, y), z in zip(list(pos.values()), DEM)
            ]
        )
...
def change_breach_location(
    breach_polygon_file, point1=[0, 0.5], point2=[0.5, 0], grid_size=100
):
...
def from_output_nc_to_txt(output_map, save_folder, seed):
    ...
    """
    # retrieve map data
    Mesh2d_face_x = get_ncmodeldata(
        file_nc=output_map, varname="Mesh2d_face_x", silent=True
    )
    Mesh2d_face_y = get_ncmodeldata(
        file_nc=output_map, varname="Mesh2d_face_y", silent=True
    )
    ...
    waterdepth = get_ncmodeldata(
        file_nc=output_map, varname="Mesh2d_waterdepth", timestep="all", silent=True
    )[:, order]
    velocity_x = get_ncmodeldata(
        file_nc=output_map, varname="Mesh2d_ucx", timestep="all", silent=True
    )[:, order]
    velocity_y = get_ncmodeldata(
        file_nc=output_map, varname="Mesh2d_ucy", timestep="all", silent=True
    )[:, order]
...
def run_simulations(
    ...
    for sim in tqdm(range(start_sim, start_sim + n_sim)):
        ...
        DEM = grid_DEM_generator(
            sim, number_grids, noise_octave, initial_octave, DEM_multiplier, initial_DEM
        )
        ...
        simulation_stats.append(
            [sim, grid_size, number_grids, simulated_time_hours, computation_time]
        )
