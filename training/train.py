...
class Trainer(object):
    ...
    def _training_step(self, model, train_loader, curriculum_epoch=1, **loss_options):
        ...
        for batch in charge_bar:
            ...
            for i in range(rollout_steps):
                ...
                loss = loss_WD(
                    preds, temp.y[:, :, i], type_loss=self.type_loss, **loss_options
                )
    ...
    def fit(self, model, train_loader, val_dataset, **temporal_test_dataset_parameters):
        assert isinstance(
            train_loader, DataLoader
        ), "Training requires train_loader to be a Dataloader object"
        ...
        try:
            for _ in progress_bar:
                ...
                self._get_spatial_analysis(
                    model, val_dataset, **temporal_test_dataset_parameters
                )

                # Model validation
                val_loss = self.spatial_analyser._get_rollout_loss(
    ...
    def _get_spatial_analysis(
        self, model, val_dataset, **temporal_test_dataset_parameters
    ):
        self.spatial_analyser = SpatialAnalysis(
            model, val_dataset, **temporal_test_dataset_parameters
        )
