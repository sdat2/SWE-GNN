...
def rollout_test(model, dataset, **temporal_test_dataset_parameters):
    ...
    assert (
        temp.time == time_start
    ), f"You're starting at time {temp.time} instead of time {time_start}"
...
def get_rollouts(model, dataset, **temporal_test_dataset_parameters):
    ...
    predicted_rollout = rollout_test(
    ...
    assert (
        real_rollout.shape == predicted_rollout.shape
