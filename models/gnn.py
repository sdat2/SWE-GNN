    ...
    def __init__(
        ...
        **base_model_kwargs
        ...
        self.static_node_features = (
            node_features - self.dynamic_node_features + self.with_WL
        )
        ...
        self.gnn_activations = Seq(
            *[activation_functions(gnn_activation, device=self.device)] * n_GNN_layers
        )
        ...
        **mlp_kwargs
        ...
        if with_filter_matrix:
            self.filter_matrix = torch.nn.ModuleList(
                [
                    nn.Linear(dynamic_node_features, dynamic_node_features, bias=False)
                    for _ in range(K + 1)
                ]
            )

    def forward(
        self, x_s: Tensor, x_t: Tensor, edge_index: Tensor, edge_attr: Tensor
    ) -> Tensor:
